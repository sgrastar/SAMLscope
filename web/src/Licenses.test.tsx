...
test('loads Java license text and exact source availability on demand', async () => {
  ...
  const fetcher = vi.fn(async (url: string) => ({ ok: true, json: async () =>
    url.includes('source-notices') ? { sources: [] } : { packages: url.includes('java-dependencies') ? [{
      file: 'example.jar', permission: {
        supplemental_notices: [{ license: 'EPL-2.0', text: 'Retained <copyright> and full terms' }],
        source_availability: { url: 'https://example.org/1.0-sources.jar', statement: 'Source is available under EPL-2.0.' },
      },
    }, { file: 'original-notice.jar', notices: [{ file: 'LICENSE', text: 'Original upstream copyright without supplemental review metadata' }] }] : [] },
  }))
  vi.stubGlobal('fetch', fetcher)
  render(<Licenses />)
  expect(fetcher.mock.calls.some(([url]) => url.includes('java-dependencies'))).toBe(false)
  const details = screen.getByText('Java copyright notices, license texts and source availability').closest('details')!
  details.open = true
  fireEvent(details, new Event('toggle'))
  expect(await screen.findByText('Retained <copyright> and full terms')).toBeTruthy()
  expect(screen.getByText('Original upstream copyright without supplemental review metadata')).toBeTruthy()
  expect(screen.getByRole('link', { name: 'Upstream source code' }).getAttribute('href')).toBe('https://example.org/1.0-sources.jar')
})
...
test('shows software scope and retained dependency notices as text', async () => {
  ...
  expect(screen.getByText(/Source notices and modification credits are retained/)).toBeTruthy()
})
